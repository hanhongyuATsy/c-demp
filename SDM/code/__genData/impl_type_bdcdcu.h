#ifndef ____BDCDCU_H__
#define ____BDCDCU_H__
...
public:
	...
	std::uint16_t DCU_SelfTorqMin;
	std::uint16_t DCU_SelfTorqMax;
/*
	BDCDCU() {}
	~BDCDCU() {}
	BDCDCU(const std::uint16_t _DCU_IdcAct,const std::uint16_t _DCU_UdcAct,const std::uint8_t _DCU_ModeSt,const std::uint8_t _DCU_GateDrvSt,const std::uint8_t _DCU_DiagErrCode,const std::uint16_t _DCU_TorqMin,const std::uint16_t _DCU_TorqMax,const std::uint8_t _DCU_HvilSt,const std::uint16_t _DCU_EOP_SpdSet,const std::uint8_t _DCU_RemoteDiagCode,const std::uint8_t _DCU_InvtTemp,const std::uint16_t _DCU_StatTemp,const std::uint8_t _DCU_TempCoolant,const std::uint8_t _DCU_VoltageT30,const std::uint8_t _DCU_FaultLampInd,const std::uint8_t _DCU_SysTempOvrInd,const std::uint16_t _DCU_Swversion,const std::uint16_t _DCU_OffsetAngle,const std::uint16_t _DCU_BootSV,const std::uint32_t _DCU_BSWVersion,const std::uint16_t _DCU_TrefFinal,const std::uint16_t _DCU_iCurqRef,const std::uint16_t _DCU_iCurdRef,const std::uint16_t _DCU_Id,const std::uint16_t _DCU_Iq,const std::uint16_t _DCU_Fault1,const std::uint16_t _DCU_Fault2,const std::uint16_t _DCU_Fault3,const std::uint16_t _DCU_Fault4,const std::uint32_t _DCU_Fault5,const std::uint32_t _DCU_Fault6,const std::uint16_t _DCU_RotTemp,const std::uint16_t _DCU_SelfTorqMin,const std::uint16_t _DCU_SelfTorqMax):DCU_IdcAct(_DCU_IdcAct),DCU_UdcAct(_DCU_UdcAct),DCU_ModeSt(_DCU_ModeSt),DCU_GateDrvSt(_DCU_GateDrvSt),DCU_DiagErrCode(_DCU_DiagErrCode),DCU_TorqMin(_DCU_TorqMin),DCU_TorqMax(_DCU_TorqMax),DCU_HvilSt(_DCU_HvilSt),DCU_EOP_SpdSet(_DCU_EOP_SpdSet),DCU_RemoteDiagCode(_DCU_RemoteDiagCode),DCU_InvtTemp(_DCU_InvtTemp),DCU_StatTemp(_DCU_StatTemp),DCU_TempCoolant(_DCU_TempCoolant),DCU_VoltageT30(_DCU_VoltageT30),DCU_FaultLampInd(_DCU_FaultLampInd),DCU_SysTempOvrInd(_DCU_SysTempOvrInd),DCU_Swversion(_DCU_Swversion),DCU_OffsetAngle(_DCU_OffsetAngle),DCU_BootSV(_DCU_BootSV),DCU_BSWVersion(_DCU_BSWVersion),DCU_TrefFinal(_DCU_TrefFinal),DCU_iCurqRef(_DCU_iCurqRef),DCU_iCurdRef(_DCU_iCurdRef),DCU_Id(_DCU_Id),DCU_Iq(_DCU_Iq),DCU_Fault1(_DCU_Fault1),DCU_Fault2(_DCU_Fault2),DCU_Fault3(_DCU_Fault3),DCU_Fault4(_DCU_Fault4),DCU_Fault5(_DCU_Fault5),DCU_Fault6(_DCU_Fault6),DCU_RotTemp(_DCU_RotTemp),DCU_SelfTorqMin(_DCU_SelfTorqMin),DCU_SelfTorqMax(_DCU_SelfTorqMax) {}
	BDCDCU(const BDCDCU &_x){
		...
		DCU_SelfTorqMin = _x.DCU_SelfTorqMin;
		DCU_SelfTorqMax = _x.DCU_SelfTorqMax;
	}
	BDCDCU(BDCDCU &&_x){
		...
		DCU_SelfTorqMin = std::move(_x.DCU_SelfTorqMin);
		DCU_SelfTorqMax = std::move(_x.DCU_SelfTorqMax);
	}
	BDCDCU& operator=(const BDCDCU &_x){
		DCU_IdcAct = _x.DCU_IdcAct;
		DCU_UdcAct = _x.DCU_UdcAct;
		DCU_ModeSt = _x.DCU_ModeSt;
		DCU_GateDrvSt = _x.DCU_GateDrvSt;
